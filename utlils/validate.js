...
function validate(){
    ...
    function handleValidateDOB(dob){
        const dobInput = dob
...
function displayCardIcon(cardType) {
  ...
  if (cardType) {
    ...
    function validateExpDate() {
        ...
        document.getElementById("expDate-error").textContent = "Expiration date must be larger than current date";
        expDateInput.focus();
        return false;
        } else {
    ...
    sssnInput.addEventListener("input", function () {
    // Remove non-digit characters and limit the input to 4 characters
